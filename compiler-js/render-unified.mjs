 ...
 * render-unified.mjs
 *
 * End-to-end rendering: TEI XML → HTML via the unified/xast handler map
 * generated by odd-to-unified.mjs.
 *
 * Pipeline:
 *   1. odd-to-unified.mjs  → tei-handlers.mjs   (ODD → handler module)
 *   2. render-unified.mjs   → rendered HTML       (this script)
 *
 * The generated handler module exports teiToHast(), which converts an xast
 * tree (parsed XML) to a hast tree (HTML AST).  This script:
 *   a) Parses the TEI XML into an xast-compatible tree
 *   b) Dynamically imports the generated handler module
 *   c) Calls teiToHast() to produce a hast tree
 *   d) Serializes the hast tree to an HTML string
 *
 * Parses the TEI with @rgrove/parse-xml (the parser unified's
 * xast-util-from-xml is built on) into an xast tree; serialises with the
 * built-in hast serialiser, or hast-util-to-html if it is installed.
 *
 * The ODD-generated stylesheet (edition.css from odd-to-css.mjs) is inlined
 * into the page, so the edition's visual styling is genuinely derived from the
 * ODD rather than from a parallel hand-maintained mapping. The only CSS this
 * script still owns is the demo page chrome and the scaffolding classes the
 * renderer itself synthesises (footnote refs/bodies, choice toggles) — those
 * are not TEI elements and therefore cannot come from the ODD.
 *
 ...
 * --tei flag. With one input the output is output/rendered-unified.html (as
 * before); with several, one <basename>.html page is written per source plus an
 * index.html linking them — the difference between a demo and a real edition.
 *
 * Usage:
 *   node render-unified.mjs --handlers <path> --tei <path|dir|glob> [--css <path>] [--out <dir>]
 *
 * Outputs:
 *   <output-dir>/rendered-unified.html           (single input)
